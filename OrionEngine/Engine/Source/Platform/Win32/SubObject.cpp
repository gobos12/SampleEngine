...
namespace Win32
{
	...
	{
		if (msg == WM_NCCREATE)  // when non-client create message, we make a new pointer to the suboject we need
		{
			...
			return pWnd->MessageHandler(hWnd, msg, wParam, lParam); // pulls message handler from inherited subobjects
		}
	...
	{
		Win32::SubObject* const pWnd = reinterpret_cast<SubObject*>(GetWindowLongPtr(hWnd, GWLP_USERDATA));
